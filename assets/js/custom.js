...
const app = (function () {
		...
		const errorToast = (message) => {
			let toaster = new Toast("Error",message);
		...
		const handSonTable = (element,options)=>{
		...
		const validator = (source)=>{
			return function isValid(value, callback) {
		...
		return {
			...
			getHandSonTable :(element,options)=>handSonTable(element,options),
			setValidator : (sourceData)=>validator(sourceData),
