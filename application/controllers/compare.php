...
//		$permission = 0;
		if ($checkPermission == true) {
			...
			if($gl_data->totalCount>0)
			{
				...
				{
					$scheduleParentMapp[$glRow->account_gl_number][]=$glRow->schedule_account;
				}
			}
...
//		var_dump($_FILES);
	...
	public function getMainScheduleAccountData()
	{
		...
		$getAccountIds = $this->Master_Model->_select($mainSetup, array('company_id' => $company_id, 'status' => 1), '*', false, null, null, 'sequence_number asc')->data;
		$dataNew = array();
		$account_array = array();
		if (count($getAccountIds) > 0) {
			foreach ($getAccountIds as $row) {
				array_push($account_array, $row->main_gl_number . "-" . $row->group_id . "-" . $row->name);
			}
		}
		if (count($getDataBranch) > 0) {
			foreach ($getDataBranch as $row1) {
				$group_data = $this->Master_Model->get_row_data('*', array('main_gl_number' => $row1->account_gl_number,'group_id'=>$row1->group_id), $mainSetup);
				$group_id = "";
				if ($group_data != null) {
					$group_id = $group_data->main_gl_number . "-" . $group_data->group_id . "-" . $group_data->name;
				}
				...
				$data1 = array($row1->schedule_account,
					...
					$group_id,
					$row1->type0,
					$row1->calculation_method, $row1->sequence_no, $row1->monitory, $divide);
				array_push($dataNew, $data1);
			}
			$data12 = array("", "", "", "");
			array_push($dataNew, $data12);
	...
	public function InsertMainScheduleAccountData()
	{
		if (!is_null($this->input->post('value'))) {
			...
			$parentAccountArr=array();
			$getAllParentAccount=$this->Master_Model->_select($mainSetup,array('company_id' => $company_id, 'status' => 1),'main_gl_number',false);
			if($getAllParentAccount->totalCount>0)
			{
				foreach ($getAllParentAccount->data as $parentaccount)
				{
					array_push($parentAccountArr,$parentaccount->main_gl_number);
				}
			}
			$main_array = array_column($value, 0);
			$main_array = array_filter($main_array);
				...
				foreach ($value as $item) {
					if ($item[0] != "0" && $item[0] != "") {
						if ($item[2] != "" && $item[2] != null) {
							...
							$account_id=0;
							if (count($group_data_ids) > 0) {
								$account_id = $group_data_ids[0];
								$group_id = $group_data_ids[1];
							}
							...
							$group_data = $this->Master_Model->get_row_data('*', $where, $groupTable);
							if ($group_data != null) {
								if(in_array($item[0],$parentAccountArr))
								{
									$response['status'] = 201;
									$response['body'] = $item[0] . " Already Exists in Parent Account. You can not add to Schedule Account";
									echo json_encode($response);
									exit();
								}
								if ($item[3] == "BS") {
								...
								$data = array(
									...
									"name" => $item[1],
									"account_gl_number" => $account_id,
									"group_id" => $group_data->id,
									"type0" =>  $item[3],
									"calculation_method" => $group_data->calculation_method,
									"sequence_no" => $group_data->sequence_no,
									"company_id" => $company_id,
									"created_by" => $user_id,
									"monitory" => $group_data->monitory_status,
									"is_divide" => $group_data->is_divide,
