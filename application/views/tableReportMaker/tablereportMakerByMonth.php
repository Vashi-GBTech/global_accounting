...
<script type="text/javascript">
	...
	function getHandsonData() {
		...
		// console.log(formData);
		$.ajax({
			url: base_url + "createTableReportMonthHandson",
			type: "POST",
			dataType: "json",
			data: formData,
			processData: false,
			...
			success: function (result) {
				...
				if (result.status == 200) {
					...
					let hiddenColumns = [];
					handson(columnsHeader, columnsRows, columnTypes, 'handsonreport', hiddenColumns, 23);
	...
	function getYearConsolidatedMonth(year) {
		...
		$.ajax({
			url: base_url + "getTableConsolidatedMonth",
			type: "POST",
			dataType: "json",
			data: formData,
			processData: false,
